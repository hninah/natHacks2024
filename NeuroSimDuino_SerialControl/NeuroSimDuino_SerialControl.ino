...
// Callback functions for LED commands
void cmd_led_red_Callback(SerialCommands* sender) {
  turnOffAllLEDs();
  digitalWrite(redLED, HIGH);
  sender->GetSerial()->println("Red LED ON");
}

void cmd_led_yellow_Callback(SerialCommands* sender) {
  turnOffAllLEDs();
  digitalWrite(yellowLED, HIGH);
  sender->GetSerial()->println("Yellow LED ON");
}

void cmd_led_green_Callback(SerialCommands* sender) {
  turnOffAllLEDs();
  digitalWrite(greenLED, HIGH);
  sender->GetSerial()->println("Green LED ON");
}

void cmd_led_off_Callback(SerialCommands* sender) {
  turnOffAllLEDs();
  sender->GetSerial()->println("All LEDs OFF");
}
...
SerialCommand cmd_led_red_("LEDRED", cmd_led_red_Callback);
SerialCommand cmd_led_yellow_("LEDYELLOW", cmd_led_yellow_Callback);
SerialCommand cmd_led_green_("LEDGREEN", cmd_led_green_Callback);
SerialCommand cmd_led_off_("LEDOFF", cmd_led_off_Callback);
...
void setup() 
{
  ...
  // Configure the LED for output and sets the initial state to off
  pinMode(redLED, OUTPUT);
  pinMode(yellowLED, OUTPUT);
  pinMode(greenLED, OUTPUT);
  turnOffAllLEDs();

  // Initialize SerialCommands and add commands
  serial_commands_.SetDefaultHandler(cmd_unrecognized);
  serial_commands_.AddCommand(&cmd_led_red_);
  serial_commands_.AddCommand(&cmd_led_yellow_);
  serial_commands_.AddCommand(&cmd_led_green_);
